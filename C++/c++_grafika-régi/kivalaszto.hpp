...
void torol(vector<OBJ*> &objektumok, int id, unsigned int &focusban)
{
	...
	if (focusban==id and objektumok.size()>2)
	{
...
void KIVALASZTO::ss(stringstream &s)
{
	...
	if (i==1) // ADD
	{
		string uj;
		getline(s,uj);
		if (uj.at(0)==32) uj = uj.substr(1, uj.size()); // Hogy számot is lehesen hozzáadni.
	...
	else if (i==2) // DEL jelenlegit
	{
		...
		return; // Visszatér a törölt elem ID-jével.
	}
	else if (i==3) // DEL ezt
	{
		int van=-1;
