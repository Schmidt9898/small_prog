...
class SZAMBEALLITO : public ABLAK
{
	public:
		...
		SZAMBEALLITO(double x, double y, double A, double B, SZIN szin, SZIN szin2, double szam=0, unsigned int size=8, bool korbe=false)
			: ABLAK(x,y,gout.twidth("0")*(size+2)+TEXT_RAHAGYAS*5+gout.cascent(),gout.cascent()*2+TEXT_RAHAGYAS*11,szin)
		{
			...
			objektumok.push_back( new SZAMBOX(2,gout.cascent()/2+TEXT_RAHAGYAS*3.5,A,B,szin,szin2,szam,size,korbe) );
		}
